...
function createQuickSort(order, dtype) {
  ...
  function compareSwap(a, b) {
    ...
    //  if (compare(el1, el2) > 0) { var t = el1; el1 = el2; el2 = t; }
      ...
      code.push(["if(", dataRead(toPointer(a)), "]>", dataRead(toPointer(b)), "){tmp=", a, ";",a,"=",b,";", b,"=tmp}"].join(""))
    }
  }
  ...
  function storePivot(mem_dest, pivot_dest, pivot) {
    if(order>1) {
      ...
      code.push(
          dataWrite(mem_dest, dataRead(pivot_dest)),
          dataWrite(pivot_dest, "pivot"+pivot))
    }
  }
  ...
  function walkPointer(ptr, pivot, body) {
    if(order.length > 1) {
      ...
      code.push(["while(", dataRead(ptr), "===pivot", pivot, "){", body, "}"].join(""))
    }
  }
  ...
  if(allocator) {
    ...
    return compiled(insertionSort, malloc, free)
  }
